...
# soft link
LN_PLUGINS="/injoy/plugins/obagent-plugins" 
CUR_PLUGINS=$DEPLOY_DIR/plugins
...
elif [ -f $CONF_DIR/log4j2-spring.xml ]; then
   LOGGING_CONFIG="-Dlogging.config=$CONF_DIR/log4j2-spring.xml"      
fi
...
echo -e "Starting the ln plugins..."
if [ ! -d "$CUR_PLUGINS" ]
then
ln -s $LN_PLUGINS $CUR_PLUGINS
fi
echo -e "ln plugins... ok!" 
